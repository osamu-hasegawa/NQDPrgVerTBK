...
Global Const gVelDirct! = -1            'S.Mの回転方向 (+1 or -1) 速度制御時 '08.3.24
'Global Const gRev2Disp As Double = 24000       '1回転あたりのパルス数 東栄サーボモーター　レゾルバ 24000Pulse/rev.
Global Const gRev2Disp As Double = 8000         '1回転あたりのパルス数 KD2002ED(ﾂﾊﾞｷ)モーター 2000*4=8000Pulse/rev.
...
Global r_pres_kousei!                  '圧力読取値の　ゼロ校正値
...
Dim fDir$, flNm$
  ...
  Open fDir & flNm For Input As #fnum
    ...
    Input #fnum, r_pres_kousei          '圧力読み取りのゼロ校正値
  Close #fnum
...
Dim fDir$, flNm$
  ...
  Open fDir & flNm For Output As #fnum
    ...
    Write #fnum, r_pres_kousei          '圧力読み取りのゼロ校正値
  Close #fnum
End Sub
...
Dim ErrNo%, EmgArm%
  ...
  ErrNo = 14: gErrMsg$(EmgArm, ErrNo) = "予備加熱上下未到達　下"
  ErrNo = 15: gErrMsg$(EmgArm, ErrNo) = "予備加熱上下未到達　上"
  End Sub
